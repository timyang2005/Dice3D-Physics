...
class DiceRenderer(private val context: Context, private val physicsWorld: PhysicsWorld, private val cameraController: CameraController) : GLSurfaceView.Renderer {
    ...
    override fun onSurfaceCreated(gl: GL10?, config: EGLConfig?) {
        ...
        GLES30.glEnable(GLES30.GL_DEPTH_TEST); GLES30.glDisable(GLES30.GL_CULL_FACE)
        GLES30.glEnable(GLES30.GL_BLEND); GLES30.glBlendFunc(GLES30.GL_SRC_ALPHA, GLES30.GL_ONE_MINUS_SRC_ALPHA)
        MatrixState.setInitStack()
        DiceType.entries.forEach { type -> diceModels[type] = DiceModel(context, type) }
        tableRenderer = TableRenderer(context); shadowRenderer = ShadowRenderer(context)
    }
    ...
    override fun onDrawFrame(gl: GL10?) {
        ...
        updateLightPosition(); MatrixState.setLightLocation(lightX,lightY,lightZ)
    ...
    private fun drawAllDice() {
        GLES30.glEnable(GLES30.GL_BLEND); GLES30.glBlendFunc(GLES30.GL_SRC_ALPHA, GLES30.GL_ONE_MINUS_SRC_ALPHA)
        for(dice in physicsWorld.diceBodies) {
            val model=diceModels[dice.diceType] ?: continue
            val transform=dice.getTransform()
            MatrixState.pushMatrix(); MatrixState.scale(1.8f,1.8f,1.8f); applyPhysicsTransform(transform); model.draw(); MatrixState.popMatrix()
            MatrixState.pushMatrix(); MatrixState.scale(1.8f,1.8f,1.8f); applyPhysicsTransform(transform); shadowRenderer?.draw(model.verticesBuffer, model.vertexCount); MatrixState.popMatrix()
        }
    }
