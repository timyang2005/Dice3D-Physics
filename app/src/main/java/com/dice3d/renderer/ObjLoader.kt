...
object ObjLoader {
    ...
    fun load(context: Context, fileName: String, scale: Float = 0.1f): ModelData? {
        ...
        try {
            ...
            while (reader.readLine().also { line = it } != null) {
                ...
                when (parts[0]) {
                    ...
                    "f" -> {
                        val indices = IntArray(3); val texIndices = IntArray(3)
                        for (i in 0..2) {
                            val fp = parts[i+1].split("/")
                            indices[i] = fp[0].toInt()-1
                            if (fp.size>1 && fp[1].isNotEmpty()) texIndices[i] = fp[1].toInt()-1
                            val vi = indices[i]
                            alvResult.add(alv[3*vi]); alvResult.add(alv[3*vi+1]); alvResult.add(alv[3*vi+2])
                            if (texIndices[i] < altexCoord.size/2) { altexResultCoord.add(altexCoord[2*texIndices[i]]); altexResultCoord.add(altexCoord[2*texIndices[i]+1]) }
            ...
            for (idx in alFaceIndex) { val avg=averageNormals(normalMap[idx]?:continue); normals[c++]=avg[0]; normals[c++]=avg[1]; normals[c++]=avg[2] }
            return ModelData(vertices, normals, texCoords, vertices.size/3)
        } catch (e: Exception) { Log.e(TAG,"Failed to load OBJ: $fileName",e); return null }
    }
